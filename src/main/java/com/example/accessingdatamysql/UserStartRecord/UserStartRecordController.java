...
public class UserStartRecordController {
    ...
    public @ResponseBody String addNewStartRecord(@RequestBody UserStartRecordRequest request){ return userStartRecordService.addNewUserStartRecord(request); }

    @PostMapping("/changeResolution")
    @Operation(summary = "금연각오 수정", description = "Parameter로 token하고 새로운 각오를 받아서 db에 있는 각오를 수정함")
    public @ResponseBody String changeResolution(@RequestParam String token, @RequestParam String resolution){
    ...
    public @ResponseBody String stopQuitting(@RequestParam String token){
